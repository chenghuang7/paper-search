...
目标更新时间为北京时间每天 **09:17**。GitHub 每小时第 17 分钟触发一次检查：09:17 之前跳过；当天 09:17 之后尚未成功检索并发布，就执行补抓与发布。成功后，当天后续检查直接跳过，不重复检索。如果某次触发遗漏或执行失败，后续小时会再次尝试。

每小时触发配置在 `.github/workflows/daily.yml`；每日起始时间在 `scripts/schedule_gate.py`。修改配置、网页或检索器并推送到 `main`，以及手动运行，都会立即执行、不受每日检查限制。Actions 定时事件可能延迟甚至被丢弃，补偿检查只能降低漏更概率，不能保证准点；不需要本地电脑开机。
